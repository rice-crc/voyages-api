...
class InterimVoyage(models.Model):
	...
	ship_construction_place = models.ForeignKey(Place,
												related_name='+',
												null=True,
												blank=True,
												on_delete=models.CASCADE)
	ship_registration_place = models.ForeignKey(Place,
												related_name='+',
												null=True,
												blank=True,
												on_delete=models.CASCADE)
	national_carrier = models.ForeignKey(Nationality,
										 related_name='+',
										 null=True,
										 blank=True,
										 on_delete=models.CASCADE)
	...
	# Outcome
	voyage_outcome = models.ForeignKey(ParticularOutcome,
									   related_name='+',
									   null=True,
									   blank=True,
									   on_delete=models.CASCADE)
	african_resistance = models.ForeignKey(Resistance,
										   related_name='+',
										   null=True,
										   blank=True,
										   on_delete=models.CASCADE)
	...
	first_place_of_landing = models.ForeignKey(Place,
											   related_name='+',
											   null=True,
											   blank=True,
											   on_delete=models.CASCADE)
	second_place_of_landing = models.ForeignKey(Place,
												related_name='+',
												null=True,
												blank=True,
												on_delete=models.CASCADE)
	third_place_of_landing = models.ForeignKey(Place,
											   related_name='+',
											   null=True,
											   blank=True,
											   on_delete=models.CASCADE)
	principal_place_of_slave_disembarkation = models.ForeignKey(
		...
		blank=True,
		on_delete=models.CASCADE)
	port_voyage_ended = models.ForeignKey(Place,
										  related_name='+',
										  null=True,
										  blank=True,
										  on_delete=models.CASCADE)
	...
	# Dates
	date_departure = models.CharField(max_length=10,
									  blank=True,
									  null=True)
	...
	imputed_national_carrier = models.ForeignKey(Nationality,
												 related_name='+',
												 null=True,
												 blank=True,
												 on_delete=models.CASCADE)
...
class InterimContributedSource(models.Model):
	...
	created_voyage_sources = models.ForeignKey(VoyageSources,
											   null=True,
...
class InterimBookSource(InterimContributedSource):
	"""
	Book/essay in book source for an interim 
	"""
	interim_voyage = models.ForeignKey(InterimVoyage,
									   null=False,
...
# 		default=VoyageDataset.Transatlantic,
	...
	def get_status_msg(self):
		decision_values = {
			ReviewRequestDecision.under_review: ('Under review'),
			ReviewRequestDecision.accepted_by_reviewer:
			_('Accepted (reviewer)'),
			ReviewRequestDecision.rejected_by_reviewer:
			_('Rejected (reviewer)'),
			ReviewRequestDecision.accepted_by_editor: ('Accepted'),
			ReviewRequestDecision.rejected_by_editor: ('Rejected'),
			ReviewRequestDecision.begun_editorial_review: ('Editor bypass')}
		response_values = {
			ReviewRequestResponse.no_reply: ('No reply'),
			ReviewRequestResponse.accepted: ('Will review'),
			ReviewRequestResponse.rejected: ('Cannot review')}
...
class EditorVoyageContribution(models.Model):
	...
	def __unicode__(self):
		return ('Editorial review of contribution')
...
class EditVoyageContribution(BaseVoyageContribution):
	...
	def __unicode__(self):
		return ('Edit an existing voyage')
...
class MergeVoyagesContribution(BaseVoyageContribution):
	...
	merged_voyages_ids = models.CharField(
		...
		help_text='The voyage_id of each Voyage being merged by this '
				  'contribution')
	help_text = ('Enter your preferred data to the right. If required use '
				  'the box for '
	...
	def __unicode__(self):
		return ('Merge two or more voyages into one')
